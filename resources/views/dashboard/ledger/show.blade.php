            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        Ledger Entries for {{ $employee->name }}
                    </div>
                    <div class="card-body">
                                        ...
                                        @php
                                            $balance +=
                                                $ledger->transaction_type == 'income'
                                                    ? $ledger->amount
                                                    : -$ledger->amount;
                                            $remainingAmount = $ledger->employee_service->id
                                                ? $remainingAmounts[$ledger->employee_service->id] ?? 'N/A'
                                                : 'N/A';
                                            $employee_service_name =
                                                $ledger->employee_service->name ??
                                                ($ledger->employee_service->employee->name .
                                                    '-' .
                                                    $ledger->employee_service->service->name ??
                                                    'N/A');
                                        @endphp
                                        <tr>
                                            ...
                                            <td><a
                                                    href="{{ route('employeeServices.edit', ['employee_service_id' => $ledger->employee_service->id]) }}">{{ $employee_service_name }}</a>
                                            </td>
                                            <td>{{ $ledger->medium }}</td>
                                            ...
                                            <td>{{ $ledger->remaining_amount ? "$employee_service_name - Remaining: $" . number_format($remainingAmount, 2) : '' }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
