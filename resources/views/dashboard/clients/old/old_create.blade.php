...
@section('content')
    ...
    <div class="duration-dropdown mt-2 p-2 border rounded">
        ...
        <input type="number" name="service_duration_quantity[${serviceName}]" min="1" placeholder="Quantity" class="form-control">
                ...
                `;
                $('.form-check').on('click', '.close-btn', function() {
                    var durationDropdown = $(this).closest('.duration-dropdown');
            ...
            function addExistingServiceToCheckbox(serviceId) {
                ...
                var checkboxHtml = `
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="${checkboxId}" name="services[]" value="${serviceId}" checked>
                        <label class="form-check-label" for="${checkboxId}">
                            ${serviceName}
                        </label>
                        ...
                        <div class="duration-dropdown">
                            <button type="button" class="btn btn-danger btn-sm ml-2 remove-btn"><i class="fa fa-close text-danger"></i></button>
                            <label for="duration-${checkboxId}">Duration:</label>
                            <select id="duration-${checkboxId}" name="service_duration[${serviceId}]" class="form-control">
                                <option value="">Select Duration</option>
                                <option value="day">Day</option>
                                <option value="week">Week</option>
                                <option value="month">Month</option>
                            ...
                            <input class = "w-auto"type="number" name="service_duration_quantity[${serviceId}]" min="1" placeholder="duration" class="form-control mt-2">
                        </div>
                    </div>
                `;
            ...
            $('#new_services_list').on('click', 'input[type="checkbox"]', function() {
                var checkbox = $(this);
                var durationDropdown = checkbox.siblings('.duration-dropdown');
            ...
            $('#existing_services_list').on('click', 'input[type="checkbox"]', function() {
                var checkbox = $(this);
                var durationDropdown = checkbox.siblings('.duration-dropdown');
                if (checkbox.is(':checked')) {
                    durationDropdown.show();
