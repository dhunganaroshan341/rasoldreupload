    ...
    <div class="container mt-5">
        ...
        <form action="{{ $formAction }}" method="POST">
            ...
            <div class="card p-4 shadow-sm">
                <!-- Expense Type -->
                <div class="mb-3 row">
                    <label for="source_type" class="col-sm-3 col-form-label">Select Expense Type:</label>
                    <div class="col-sm-9">
                ...
                <div class="mb-3 row d-none">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="expense_source" id="expense_source" class="form-control"
                            ...
                            value="{{ old('expense_source', $expense->expense_source ?? 'not_used') }}">
                    </div>
                </div>
                    ...
                    <div class="col-sm-9">
                        <select class="form-control select2" name="medium" id="medium" required>
                            <option value="cash" {{ old('medium', $expense->medium ?? '') == 'cash' ? 'selected' : '' }}>
                                Cash</option>
                            <option value="cheque"
                                {{ old('medium', $expense->medium ?? '') == 'cheque' ? 'selected' : '' }}>Cheque</option>
                            <option value="mobile_transfer"
                                {{ old('medium', $expense->medium ?? '') == 'mobile_transfer' ? 'selected' : '' }}>Mobile
                                Transfer</option>
                            <option value="other"
                                {{ old('medium', $expense->medium ?? '') == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                </div>
